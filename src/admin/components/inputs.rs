...
impl Component for TextInput {
    ...
    fn on_blur(&mut self, state: &mut Self::State, elements: Elements<'_, '_>, context: Context<'_, Self::State>) {
...
pub trait InputReceiver {
    ...
    #[allow(dead_code)]
    fn _on_focus(&mut self, state: &mut InputState, _: Elements<'_, '_>, mut context: Context<'_, InputState>) {
    ...
    #[allow(dead_code)]
    fn _on_blur(&mut self, state: &mut InputState, _elements: Elements<'_, '_>, mut context: Context<'_, InputState>) {
        state.cursor_char.set("".to_string());
    ...
    fn add_character(&mut self, char: char, state: &mut InputState, mut context: Context<'_, InputState>) {
        let mut input = state.input.to_mut();
        let Some(cursor_position) = state.cursor_position.to_number() else {
            return;
        };
    ...
    fn delete(&self, state: &mut InputState, mut context: Context<'_, InputState>) {
        ...
        state.cursor_prefix.set(input.chars().take(pos).collect::<String>());
    ...
    fn backspace(&mut self, state: &mut InputState, mut context: Context<'_, InputState>) {
        ...
        state.cursor_prefix.set(input.chars().take(new_pos).collect::<String>());
    ...
    fn move_cursor_down(&self, state: &mut InputState) {
        ...
        state.cursor_char.set(input.chars().nth(0).unwrap_or(' ').to_string());
