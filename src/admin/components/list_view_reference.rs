...
impl ProjectWindow {
    fn move_cursor_down(&self, state: &mut ProjectWindowState) {
        ...
        self.update_project_list(first_index.into(), last_index.into(), new_cursor.into(), state);
    }
...
impl ProjectWindow {
    pub fn new() -> Self {
        ProjectWindow { project_list: vec![] }
    }
...
impl From<ProjectState> for Project {
    fn from(project_state: ProjectState) -> Self {
        ...
        request_states.iter().for_each(|req| {
            ...
            request_state_headers.iter().for_each(|request_state_header| {
